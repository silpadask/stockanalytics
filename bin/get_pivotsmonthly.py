...
import pymysql
import conf_reader
print('get pivotmonthly execution started...')
...
for sym in lst:
    count=count+1
    cur.execute("select * from nsedailybhavhist where timestamp between (select date_sub(max(timestamp), interval 30 day) from nsedailybhavhist) and (select max(timestamp) from nsedailybhavhist) and series='EQ' and symbol="+"'"+sym+"'"+" order by timestamp;")
